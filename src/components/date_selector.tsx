...
export default function DateSelector({ setDate }: Props) {
    ...
    useEffect(() => {
        setDate(days[1].toISOString().split("T")[0]);
    }, []);
    ...
    return (
        <div className="w-full bg-white border-b border-slate-200">
                ...
                <div className="relative flex-1 px-5 overflow-hidden">
                    <SliderWrapper className="gap-2">
                        {days.map((date, index) => {
                            ...
                            const isClickable = index > 0 && index < 10; // today (index 0) disabled
            ...
            <div className="flex items-center gap-2 px-4 pb-3 text-xs text-slate-500">
                ...
                <span>Appointments can only be booked at least one day in advance. Same-day booking isn't available.</span>
            </div>
        </div>
    )
}